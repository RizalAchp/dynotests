...
__IO uint8_t v_start_bit = DYNO_STOPPED;
...
static int8_t CDC_Control_FS(uint8_t cmd, uint8_t *pbuf, uint16_t length) {
    ...
    case CDC_GET_LINE_CODING:
        pbuf[0] = (uint8_t)(USBD_LC.bitrate);
        pbuf[1] = (uint8_t)(USBD_LC.bitrate >> 8);
        pbuf[2] = (uint8_t)(USBD_LC.bitrate >> 16);
        pbuf[3] = (uint8_t)(USBD_LC.bitrate >> 24);
    ...
    case CDC_SET_CONTROL_LINE_STATE: {
        ...
        if (((((USBD_SetupReqTypedef *)pbuf)->wValue & 0x0001) != 0) && v_start_bit) {
